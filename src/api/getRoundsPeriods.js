...
const getRoundsPeriods = (methods) => new Promise((resolve) => {
  Promise.all([
    ...
    const startTime = responces[2] !== '0' ? (new Date(responces[2] * 1000)).toLocaleString() : null;
    const dateRounds = [];
    ...
    for (let indexTurn = 0; indexTurn < groupSize; indexTurn += 1) {
      ...
      if (indexTurn === 0) {
        dateRounds.push({
          round,
          startDate: startTime,
          endDate: new Date((Number(responces[2]) + payTime - 1) * 1000).toLocaleString(),
        });
      } else {
        dateRounds.push({
          round,
